            ...
            <div key={`word-${idx}`} className="flex">
              {word.icon && (
                <motion.span
                  initial={{}}
                  className={cn(
                    `dark:text-white text-black opacity-0 hidden align-middle`,
                    word.className,
                  )}
                >
                  {<div className="flex justify-center content-center items-center"><word.icon className="w-6 h-6 mr-2" /></div>}
                </motion.span>
              )}
  ...
  return (
    <div className={cn("text-base sm:text-xl md:text-3xl lg:text-5xl font-bold text-center", className)}>
      ...
      <motion.span
        ...
        className={cn("flex rounded-sm w-[4px] h-4 md:h-6 lg:h-10 bg-blue-500", cursorClassName)}
      ></motion.span>
    </div>
  )
}
